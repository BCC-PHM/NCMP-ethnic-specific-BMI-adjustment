...
```{r demographics}
all_records <- nrow(ncmp_data_period_8)

ncmp_data_period_8 |> 
  count(Gender, School_Year, Ethnicity_Hudda) |> 
  ungroup() |> 
  group_by(Gender, School_Year) |> 
  mutate(denom = sum(n),
         pct = n/denom*100,
         display = paste0(n, " (", round(pct, 1), "%)")) |> 
  select(-denom, -n, -pct) |> 
  arrange(School_Year) |> 
  pivot_wider(names_from = c("Gender", "School_Year"),
              values_from = "display") |> 
  knitr::kable(col.names = c("Ethnicity", "Girls", "Boys", "Girls", "Boys")) |> 
